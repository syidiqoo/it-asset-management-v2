...
import { filterDepartments } from "@/lib/master-data"
import type { DepartmentNode } from "@/lib/types"

const CSV_COLUMNS = ["Name", "Parent"]

export function DepartmentView({ values }: { values: FilterValues }) {
  ...
  const filtered = React.useMemo(
    () => filterDepartments(store.departments, values),
    [store.departments, values]
  )
  ...
  const exportCsv = () => {
    const nodes = flattenDepartments(filtered)
